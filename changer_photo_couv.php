...
require 'connection_bdd.php';
    ...
    if (isset($_FILES['photo']['tmp_name'])) {
        ...
        define('TARGET', 'photo_couv/'); // Repertoire cible
                ...
                if ($info_img[2] >= 1 && $info_img[2] <= 14) {
                    ...
                    if (($info_img[0] <= WIDTH_MAX) && ($info_img[1] <= HEIGHT_MAX) && (filesize($_FILES['photo']['tmp_name'])) <= MAX_SIZE) {
                        ...
                        if (isset($_FILES['photo']['error']) && UPLOAD_ERR_OK === $_FILES['photo']['error']) {
                            ...
                            $insert_post = $bdd->prepare('UPDATE membres SET photo_couv = :num_image WHERE id = :id');
...
?>
    ...
    <div id="post_couv">
        <h1 id="titre_photo_couv">Change ta photo de couverture</h1>
        <?php
            $change_couv = $bdd->query('SELECT photo_couv FROM membres WHERE id = '. $_SESSION['id'] .'');
            $change_couv_pic = $change_couv->fetch();
        ?>
        <img src="photo_couv/<?php echo $change_couv_pic['photo_couv'] ?>" id="change_photo_couv">
